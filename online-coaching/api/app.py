...
def get_workouts(uni):
  ...
  return [{
      ...
      "workout_type": row[3],
      "avg_power": row[4],
      "avg_stroke_rate": row[5],
      "avg_stroke_length": row[6],
      "avg_pulse": row[7],
      "avg_energy_per_stroke": row[8],
      "avg_500m_time": row[9],
      "leg_2_avg_power": row[10] if not pd.isnull(row[8]) else "",
      "leg_2_avg_pulse": row[11] if not pd.isnull(row[9]) else "",
      "leg_3_avg_power": row[12] if not pd.isnull(row[10]) else "",
      "leg_3_avg_pulse": row[13] if not pd.isnull(row[11]) else "",
      "decoupling_rate": row[14] if not pd.isnull(row[12]) else "",
      "rpe": row[15],
      "description": row[16],
      "imse": row[17],
      "tss":"",
